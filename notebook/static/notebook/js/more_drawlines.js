...
define([
    ...
    var load = function (keyboard_manager, notebook) {
        $('#draw-lines-menu').click(function () {

            var form_variable = $('<div/>').addClass('form-group')
                .append(
                    $('<label/>').addClass('col-sm-2')
                        .addClass('control-label')
                        .attr('for', 'draw_lines_variable')
            ...
            dialog.modal({
                ...
                buttons: {
                    'Run': {
                        ...
                        click: function () {
                            var Value = {
                                "variable": $("#draw_lines_variable").val(),
                                "x_field": $("#draw_lines_x_field").val(),
                                "y_field": $("#draw_lines_y_field").val(),
                                "target": "lines",
                            };
                            ...
                            if (Value.variable === "" || !/^[a-zA-Z_][0-9a-zA-Z_]*$/.test(Value.variable)) {
                                ...
                                $("#draw_lines_form div label").css("color", "#000");
                            ...
                            if (Value.x_field === "") {
                                content_1 = '# Draw Lines\n';
                                content_2 = 'sjs_load_data.draw_lines({0},y_field="{1}")\n'.format(Value.variable,Value.y_field);
                                content = content_1 + content_2
                            } else {
                                content_1 = '# Draw Lines\n';
                                content_2 = 'sjs_load_data.draw_lines({0},x_field="{1}",y_field="{2}")\n'.format(Value.variable,Value.x_field,Value.y_field);
                                content = content_1 + content_2
                            }
